...
async function loadPPAndHostedJS(type) {
  ...
  try {
    ...
    let shouldRenderStandaloneCard = false;

    if (isVaulting && customerId && components === "hosted-fields") {
      let vaultTokenRespJson = await fetch("/pcp-get-vault-token", {
        method: "POST",
        headers: {
      ...
      let vaultTokenResp = await vaultTokenRespJson.json();
...
function save() {
  ...
  if (isVaulting) localStorage.setItem("customerId", customerId);
